...
def _eval_amazon(ctx, X_train, y_train):
    from data_prep.amazon import build_head_binary, train_binary, evaluate_binary
    head = build_head_binary(feat_dim=X_train.shape[1], device="cpu")
    train_binary(head, [X_train], [y_train.astype(np.float32)], epochs=15)


    return float(evaluate_binary(head, ctx.amazon_target_sources, split="train"))
...
def compute_budget(ctx, splice_subset, dataset_type,
    ...
    if dataset_type == "acs":
        dataset = ctx.dataset
        ids = ctx.source_list if ctx.source_list is not None else list(ctx.source_X[dataset])
        all_rows = {s: len(ctx.source_X[dataset][s]) for s in ids}
...
def _k_center_indices(ctx, X, b, seed=42):
    np.random.seed(seed)
    ds = ctx.dataset
    n         = len(X)
    ...
    b    = min(b, n)
    for _ in tqdm(range(b - 1), desc="k-Center", unit="pt", leave=False):
        c        = X[selected[-1]]
...
def k_center_coreset(ctx, source_list, budget, dataset_type="acs", seed=42):
    ...
    idx    = _k_center_indices(ctx, X_pool, budget, seed=seed)
...
def _get_val_grad_amazon(ctx, model):
    from data_prep.amazon import get_target_split, compute_target_gradient_averaged
