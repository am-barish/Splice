...
from core.metrics import get_profit, get_gain, get_cost
...
def construction(ctx, source_list, selected, max_profit, k, smax=None):
    optimal = selected.copy()
    remaining = [x for x in source_list if x not in selected]

    if smax is not None and len(selected) >= smax:
...
def local_search(ctx, source_list, selected, max_profit, k, smax=None):
    changed = True
    while changed:
        changed = False
        for src in selected:
...
def grasp(ctx, source_list, num_repetitions, k, smax):
    ctx.covered = {}
    start = time.time()
    optimal = []
    max_profit = -1_000_000
    for _ in tqdm(range(num_repetitions), desc="GRASP"):
        sel, profit = construction(ctx, source_list, [], -100_000, k, smax=smax)
        sel, profit = local_search(ctx, source_list, sel, profit, k, smax=smax)
    ...
    if smax is not None and len(optimal) > smax:
        raise AssertionError(
            f"GRASP returned {len(optimal)} sources against budget {smax}")
