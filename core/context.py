...
class ExperimentContext:
    ...
    val_X_scaled: Dict[str, np.ndarray] = field(default_factory=dict)
    val_y: Dict[str, np.ndarray] = field(default_factory=dict)
    val_sens_masks: Dict[str, Dict[int, np.ndarray]] = field(default_factory=dict)
    ...
    def get_metric(self, key: Any):
        with self._cache_lock:
            return self.metrics_dict[self.dataset].get((self.eval_split, key))

    def set_metric(self, key: Any, value: Any):
        with self._cache_lock:
            self.metrics_dict[self.dataset][(self.eval_split, key)] = value
            self.covered[key] = 1

    def has_metric(self, key: Any):
        with self._cache_lock:
            return (self.eval_split, key) in self.metrics_dict[self.dataset]
    ...
    def eval_arrays(self):
        ...
        if self.eval_split == "val":
            if d not in self.val_X_scaled:
                raise RuntimeError(
                    f"No validation split for {d}. Build it in "
                    f"build_source_arrays() — selection must not read test.")
            return (self.val_X_scaled[d], self.val_y[d],
                    self.val_sens_masks.get(d, {}))
        if self.eval_split == "test":
            return (self.test_X_scaled[d], self.test_y[d],
                    self.test_sens_masks.get(d, {}))
