...
def build_source_arrays(ctx, datasets):
    ...
    for dataset in datasets:
        ...
        X_full = scaler.transform(test_df[model_feat_cols].values).astype(np.float32)
        y_full = test_df[target].values
        y_full = y_full.astype(np.float32) if dataset == "ACSTravelTime" \
            else y_full.astype(np.int8)
        sens_full = test_df[sens_col].values if (sens_col and sens_col in test_df.columns) else None

        val_idx, test_idx = get_split(
            dataset, y_full, sens=sens_full,
            is_regression=(dataset == "ACSTravelTime"))

        def _masks(sens_vals):
            if sens_vals is None:
                return {}
            return {int(g): (sens_vals == g) for g in np.unique(sens_vals)}

        ctx.val_X_scaled[dataset]  = X_full[val_idx]
        ctx.val_y[dataset]         = y_full[val_idx]
        ctx.val_sens_masks[dataset]  = _masks(sens_full[val_idx] if sens_full is not None else None)

        ctx.test_X_scaled[dataset] = X_full[test_idx]
        ctx.test_y[dataset]        = y_full[test_idx]
        ctx.test_sens_masks[dataset] = _masks(sens_full[test_idx] if sens_full is not None else None)
...
def setup_acs_context(dataset, *, data_dir = ".", gain_lambda = 10.0, method = "normal", cost_type = "zero", limit = 100_000, k_max = 15, source_list = None, seed= 42):
    ...
    metrics_dict = _load("METRICS_DICT_7Apr.pkl")
...
def setup_wilds_context(*, data_dir = ".", gain_lambda = 0.0, method = "gradmatch", limit= 100_000, source_list = None, seed = 42):
    from data_prep import wilds as _wn
    _wn.init_wilds()
    from data_prep.wilds import get_grads
    from surrogates.dataprofiles_unstructured import (
    ...
    if method in ("dp", "hybrid"):
        ctx.kurt = _load("Wilds_target_profile_val.pkl")
        history = _load("history_list_wilds_val.pkl")
...
def setup_amazon_context(*, data_dir = ".", gain_lambda = 0.0, method = "gradmatch", limit = 100_000, source_list = None, seed = 42):
    from data_prep.amazon import get_amazon_info, amazon_grad_match, get_target_split
    from surrogates.dataprofiles_unstructured import (
        MetaSurrogatePipeline,
    ...
    for cat in target_cats:
        feats, labels, splits = ctx.amazon_target_sources[cat]
        m = splits == "val"
        v_feats.append(torch.from_numpy(feats[m]).float())
        v_labels.append(torch.from_numpy(np.asarray(labels)[m]).long())

    ctx.source_pool["Amazon"] = source_pool_amazon
    ctx.test_dict["Amazon"] = {"feats": torch.cat(v_feats),
                            "labels": torch.cat(v_labels)}


    ctx.dp_dict["Amazon"] = {}
    if method in ("dp", "hybrid"):
        ctx.prev = _load("Amazon_target_profile_val.pkl")
        history  = _load("history_list_amazon_val.pkl")
        surrogate = MetaSurrogatePipeline()
        surrogate.train_surrogate(ctx, history)
...
def run_experiment(ctx, algorithm, smax = None, parallel = False, n_jobs = -1, seed = 42):
    ...
    if algorithm == "splice":
        from core.splice import get_best_subset
    ...
    if algorithm == "grasp":
        from baselines.grasp import grasp
        profit, subset, _, _, elapsed, explored, profits = grasp(ctx, source_list, 20, 5, smax=_smax)
        return dict(profit=profit, subset=subset, time_elapsed=elapsed,
                    models_explored=explored, profits=profits)

    if algorithm == "dsdm":
        from baselines.dsdm import get_dsdm_result
        profit, subset, _, _, elapsed, explored = get_dsdm_result(ctx, source_list, 500, smax=_smax)
        return dict(profit=profit, subset=subset, time_elapsed=elapsed,
                    models_explored=explored)

    if algorithm == "greedy":
        from baselines.classical import greedy
        profit, subset, _, _, elapsed, explored = greedy(ctx, source_list, _smax)
        return dict(profit=profit, subset=subset, time_elapsed=elapsed,
                    models_explored=explored)

    if algorithm == "brute_force":
        from baselines.classical import brute_force
        profit, subset, pct, rank, elapsed, explored = brute_force(ctx, source_list)
        return dict(profit=profit, subset=subset, time_elapsed=elapsed,
                    models_explored=explored, percentile=pct, rank=rank)

    if algorithm == "random":
        from baselines.classical import random_subset
        profit, subset, _, _, elapsed, explored = random_subset(ctx, source_list)
        return dict(profit=profit, subset=subset, time_elapsed=elapsed,
                    models_explored=explored)

    if algorithm == "all_sources":
        from baselines.classical import all_sources_gain
        profit, subset, _ = all_sources_gain(ctx, source_list)
        return dict(profit=profit, subset=subset)

    if algorithm == "single_source":
        from baselines.classical import single_source_gain
        profit, subset, _ = single_source_gain(ctx, source_list)
        return dict(profit=profit, subset = subset)
    ...
    if algorithm in ("k_center_coreset", "gradmatch_point"):
        ...
        from baselines.coresets import k_center_coreset, gradmatch_point
