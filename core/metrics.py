...
def santos_tables_gain(ctx, indices):
    ...
    X_ev = ctx.X_t if ctx.eval_split == "test" else ctx.X_val
    y_ev = ctx.y_t if ctx.eval_split == "test" else ctx.y_val
    f1_selected = train_eval(X_selected, y_selected, X_ev, y_ev)
    return f1_selected
...
def get_scores(ctx, source_subset, problem):
    ...
    if ctx.source_X and dataset in ctx.source_X:
        ...
        X_test, y_test, sens_masks = ctx.eval_arrays()
        ...
        if problem == "classification":
            ...
            sp, tpr, pp = _compute_fairness_from_masks(y_pred, y_test, sens_masks)
...
def get_gain(ctx, source_subset):
    from data_prep.wilds import get_subset_acc
    from data_prep.amazon import get_subset_score
...
def subset_score(ctx, indices):
    from data_prep.amazon import estimated_gain_text
    from data_prep.wilds import subset_grad
...
def get_marginal_profit_dp(ctx, s1, s2):
    from surrogates.dataprofiles import get_data_profile_difference
    from surrogates.dataprofiles_unstructured import get_marginal_profit_dp_unst
