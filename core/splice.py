...
from core.metrics import get_profit, get_grad_profit, get_marginal_profit_dp
...
def get_best_subset(ctx, source_list, smax, parallel = False, n_jobs = -1):
    ...
    if parallel:
        ...
        def _worker(i):
            ...
            worker_ctx = copy.deepcopy(ctx)
            worker_ctx.profits = [[0, 0, 0]]
            subset, profit = get_profits(worker_ctx, source_list, i)


            return subset, profit, list(worker_ctx.covered.keys()), worker_ctx.profits

        covered_union = dict(ctx.covered)
        merged_profits = list(ctx.profits)
        ...
        with Parallel(n_jobs=n_jobs, prefer="processes", return_as="generator_unordered") as parallel_pool:
            ...
            for subset, profit, covered_keys, worker_profits in tqdm(
                    results_gen, total=smax, desc="Splice (Parallel)"):
                if profit > maxprofit:
                    maxprofit = profit
                    best_subset = subset.copy()


                for _k in covered_keys:
                    covered_union[_k] = 1
                merged_profits.extend(p for p in worker_profits if p[0] != 0)

        ctx.covered = covered_union
        ctx.profits = sorted(merged_profits, key=lambda p: p[2])

        if len(covered_union) > ctx.limit:
            pass
    else:
        results = [
