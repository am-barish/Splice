...
def wilds_setup():
    ...
    target_full_df["label"] = target_full_df["category_id"].map(label2idx)
    source_df["label"] = source_df["category_id"].map(label2idx)


    counts = target_full_df["label"].value_counts()


    critically_rare_labels = counts[counts < 4].index


    rare_df = target_full_df[target_full_df["label"].isin(critically_rare_labels)].copy()


    val_forced_all = rare_df.groupby("label").sample(n=1, random_state=42)


    test_forced_all = rare_df.drop(val_forced_all.index)


    remaining_df = target_full_df.drop(rare_df.index)
    ...
    target_val_df = pd.concat([val_split_df, val_forced_all])
    target_test_df = pd.concat([test_split_df, test_forced_all])
...
def train_on_subset(loc_indices, source_feats, source_labels, source_locs, num_classes, device, epochs=15):
    ...
    current_sources = [unique_locs[i] for i in sorted(loc_indices)]
...
def evaluate_on_target(head, test_feats, test_labels, device):
    ...
    with torch.no_grad():
        ...
        active_labels = np.unique(targets)


        macro_f1 = f1_score(targets, preds, labels=active_labels, average='macro')
...
num_classes = None
s_feats = s_labels = s_locs = None
v_feats = v_labels = None
t_feats = t_labels = None

_INITIALIZED = False

def init_wilds(force=False):
    global _INITIALIZED, num_classes, s_feats, s_labels, s_locs
    global v_feats, v_labels, t_feats, t_labels
    if _INITIALIZED and not force:
        return
    n_cls, data_root, source_df, val_df, test_df, _ = wilds_setup()
    num_classes = n_cls
    resnet = models.resnet18(weights=models.ResNet18_Weights.IMAGENET1K_V1)
    resnet.fc = nn.Identity(); resnet.to(device)
    s_feats, s_labels, s_locs = precompute_location_features(
        source_df, data_root, resnet, device, "source.pt")
    v_feats, v_labels, _ = precompute_location_features(
        val_df, data_root, resnet, device, "target_val.pt")
    t_feats, t_labels, _ = precompute_location_features(
        test_df, data_root, resnet, device, "target_test.pt")
    _INITIALIZED = True

def get_subset_acc(subset, ctx):
    if not _INITIALIZED: init_wilds()
    torch.manual_seed(42)
    np.random.seed(42)
    head = train_on_subset(subset, s_feats, s_labels, s_locs, num_classes, device)
    if getattr(ctx, "eval_split", "val") == "test":
        score = evaluate_on_target(head, t_feats, t_labels, device)
    else:
        score = evaluate_on_target(head, v_feats, v_labels, device)
