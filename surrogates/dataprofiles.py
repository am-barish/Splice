...
from core.metrics import estimate_bayes_error_fast
...
def _build_profile_for_subset(ctx, subset, target_col, sens_col):
    ...
    if ctx.source_X and ctx.dataset in ctx.source_X:
        ...
        sample_n = min(len(X_all), max(1000, int(0.01 * len(X_all))))
...
DP_META_SOURCE_CAP = {"Scaled_Pubcov": 50}


def train_dp_surr_model(ctx, train_sources, test_source, datasets, metric, n_jobs= 3):
    result = {}
    np.random.seed(42)
    random.seed(42)
    ...
    for x in datasets:
        ...
        N = min(len(train_sources[x]), DP_META_SOURCE_CAP.get(x, len(train_sources[x])))
        ...
        if dp_diffs and len(dp_diffs[0]) != len(col_names):
            raise ValueError(
                f"profile width mismatch for {x}: _build_profile_for_subset "
                f"produced {len(dp_diffs[0])} features but col_names lists "
                f"{len(col_names)}. These are built under different "
                f"conditions -- the profile appends base_rate/bayes_error "
                f"only when ctx.is_fairness_dataset, while col_names adds "
                f"them for any classification task. Check "
                f"ExperimentContext.is_fairness_dataset for {x!r}.")
